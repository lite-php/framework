...
class PDO_Statement extends PDOStatement
{
     ...
     * @param DB $db
     */
    protected function __construct(PDO_Model $db)
    {
        $this->db = $db;
    }
     ...
     * @param int $fetch_from Fetch data from next or last fetched row. PDO_Model::FETCH_FROM_NEXT_ROW or PDO_Model::FETCH_FROM_LAST_ROW
     * @return object|NULL
     */
    function fetchInto($object, $from_table = "", $fetch_from = PDO_Model::FETCH_FROM_NEXT_ROW)
    {
        if ($from_table == "") {
            $this->db->setFetchTableNames(0);
            $this->setFetchMode(PDO_Model::FETCH_INTO, $object);
            return $this->fetch();
        } elseif ($fetch_from == PDO_Model::FETCH_FROM_NEXT_ROW) {
            $this->setFetchMode(PDO_Model::FETCH_ASSOC);
     ...
     * This is shortcut for fetchInto($object, $from_table, PDO_Model::FETCH_FROM_LAST_ROW);
     *
     * @param object $object
    ...
    {
        return $this->fetchInto($object, $from_table, PDO_Model::FETCH_FROM_LAST_ROW);
    }
    ...
    function fetchCollection($class_name = "stdClass")
    {
        ...
        return $this->fetchAll(PDO_Model::FETCH_CLASS, $class_name);
    }
